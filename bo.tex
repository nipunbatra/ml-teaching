...
\begin{frame}{Motivation}
	We will first start with an example which is a manifestation of optimization problems we would encounter in the wild.
	
	Our goal is to mine for gold in a new, unknown land. For now, let us make a simplifying assumption, the gold content lies in a one-dimensional space, i.e., we are talking gold distribution only about a line.
	
	We would like to find the location with the maximum gold content for us to dug out.
\end{frame}
%\newcounter{iter}

...
\begin{frame}{Why is it easier to optimize the acquisition function?}
	There are a lot more acquisition functions available possible that discussed above.
	
	But one valid question that might arise it why are we replacing one optimization problem with a sequence of optimization problems. How is this any better than the last problem?
...
\begin{frame}{Hyperparameter Tuning}
	...
	\begin{center}
		\includegraphics[width=0.7\linewidth]{bo-images/gifs/moons}
	\end{center}
\end{frame}

\begin{frame}
	\urldef\intuition\url{https://stackoverflow.com/questions/35848210/support-vector-machine-what-are-c-gamma}
	
	Let us now try to learn the classification problem above using a Support Vector Machine (SVM). We are aware that an SVM takes in two important hyperparameters, \footnote{For intuition look at this stack overflow answer - \intuition}
	
	\begin{enumerate}
		\item $\gamma$ -- modifies the behavior of the SVM's kernel. Intuitively it is a measure of how far the influence of a single training example reaches.
		
		\item $C$ -- modifies the slackness of the classification, the higher the CC is, the more sensitive is SVM towards the noise.
	\end{enumerate}
\end{frame}

\begin{frame}
	Let us apply Bayesian Optimization to learn the best hyperparameters for this classification task. The optimum values for $<C, \ \gamma>$ have been found via running grid search at a high granularity. \footnote{Note: the surface plots you see for the Ground Truth Accuracies below were calculated for each possible hyperparameter for showcasing purposes only. We don't have these values in real applications.}	
\end{frame}

\forloop{iter}{0}{\value{iter} < 10}%
{%
	\begin{frame}{\theiter \ $|$ SVM Hyperpramater tuning with PI}
		\begin{center}
			\includegraphics[width=\linewidth]{bo-images/pngs/PI3d/0.05/\theiter}
		\end{center}
...
\forloop{iter}{0}{\value{iter} < 10}%
{%
	\begin{frame}{\theiter \ $|$ SVM Hyperpramater tuning with EI}
		\begin{center}
			\includegraphics[width=\linewidth]{bo-images/pngs/EI3d/0.0001/\theiter}
		\end{center}
		Above we see a gif showing the work of the Expected Improvement acquisition function in finding the best hyperparameters.
	\end{frame}
}

\begin{frame}{Embracing Bayesian Optimization}
	...
	There are a plethora of Bayesian Optimization libraries available. Few of them can be found below.
